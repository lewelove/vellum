...
pub async fn execute_collect(payload: &ActionPayload) -> Result<()> {
    ...
    match target {
        TargetUrl::DiscogsMaster(id) => {
            fetch_and_fill_discogs(id, &mut album_data).await?;
        }
        TargetUrl::DiscogsRelease(id) => {
            let master_id = api::fetch_discogs_master_from_release(id).await?;
            fetch_and_fill_discogs(master_id, &mut album_data).await?;
        }
    }
...
async fn fetch_and_fill_discogs(id: u64, data: &mut AlbumData) -> Result<()> {
