Create generic `/api/config/scripts/{name}` that is sent to rust server with json in HTTP request body. Rust reads the `commands.{name}.path` value in config.toml -> passes the json received by ui to executable stdin -> executes it.
